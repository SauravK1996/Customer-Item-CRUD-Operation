...
namespace Customer_Item_CRUD_API.Controllers
{
    ...
    public class SalesController : ControllerBase
    {
        ...
        public IActionResult DeleteSalesItem(decimal itemId, decimal salesOrderId)
        {
            ...
            if (!(_repository.GetTabsodataOnSordid(salesOrderId).Count() > 0))
            {
        ...
        //PUT: api/Sales/2 --update item price from the Tabsodata as well as Taborder
        [HttpPut("{id}")]
        public IActionResult UpdateSalesItem(int id, [FromBody] ItemDto itemDto)
        {
            var itemRate = decimal.Parse(itemDto.ItemRate);
            var salesOrderId = decimal.Parse(itemDto.Sordid);
            
            var salesOrderItemData = _repository.GetSalesOrderItem(id, salesOrderId);
            ...
            foreach(var salesOrderItem in _repository.GetSalesOrderDataOnItemId(decimal.Parse(itemDto.ItemId)))
            {
                var salesOrder = _repository.GetSalesOrder(salesOrderItem.Sordid);
