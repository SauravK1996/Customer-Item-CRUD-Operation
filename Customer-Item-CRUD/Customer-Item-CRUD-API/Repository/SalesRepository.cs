...
namespace Customer_Item_CRUD_API.Repository
{
    ...
    public class SalesRepository : ISalesRepository
    {
        ...
        {
            var tabsodatas = _db.Tabsodatas.Where(i=>i.Itemid == tabsodata.Itemid).ToList();
            foreach(var data in tabsodatas)
            {
                data.Itemrate = tabsodata.Itemrate;
            }
            
            var itemTable =  _db.Tabitems.FirstOrDefault(a => a.Itemid == tabsodata.Itemid);
            itemTable.Itemrate = tabsodata.Itemrate;
        ...
        public ICollection<Tabsodata> GetSalesOrderDataOnItemId(decimal ItemId)
        {
            return _db.Tabsodatas.Where(i => i.Itemid == ItemId).ToList();
        }

        public ICollection<Tabsodata> GetTabsodataOnSordid(decimal sordid)
        {
            return _db.Tabsodatas.Where(sid => sid.Sordid == sordid).Where(d=>d.Dataexst=="ext").ToList();
        }
    }
}
